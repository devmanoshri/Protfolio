                ...
                <div className='flex-1'>
                    ...
                    <ul className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl'>
                        {infoList.map(({ icon, iconDark, title, description },index) => (
                            <li key={index} className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:bg-(--light-hover) hover:-translate-y-1 duration-500 shadow shadow-black'>
                    ...
                    <ul className='flex items-center gap-3 sm:gap-5'>
                        {
                            toolsData.map((tool,index)=>(
                                <li key={index} className='flex item-center justify-center w-12 sm:w-14 aspect-square border border-gray-400 rounded-lg curson-pointer hover:-translate-y-1 duration-500'>
